...
Write a program that finds the approximate grade level needed to comprehend some text.
...
* First, your program must prompt the user for a string of text.
...
* Your program should print `"Grade X"` where `X` is the grade level found by the Coleman-Liau formula, rounded to the nearest integer.
...
Next, you should count the number of letters that show up in the text. Modify the `num_letters()` function so that instead of returning `0`, it returns the number of letters in the text. Print the result to see if it's working.
...
Modify the `count_words()` function so that it counts the number of words in the text. As before, print the result to see if it's working.
...
**Don't forget to remove the three `print()` statements in the three functions!**
